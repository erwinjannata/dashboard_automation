...
apex_check.var = tk.BooleanVar(value=False)
apex_check['variable'] = apex_check.var
...
show_password_apex = tk.Checkbutton(
    root, text="Tampilkan password", onvalue=True, offvalue=False, command=toggle_check, background='white', state='disabled')
...
schedule_btn = ttk.Button(
    root, text="Jadwalkan Proses", state=tk.NORMAL, width=26)
schedule_btn.grid(row=13, column=0, pady=5, padx=5, sticky='e')

# Setting Button
setting_btn = ttk.Button(
    root, text="Settings", command=lambda: open_setting(rootWindow=root), state=tk.NORMAL, width=26)
